...
### Project overview
Project ‘RAD’ Alder explores the role of resident leaf-associated microbes prescence and absence  (i.e,present (P) vs. gamma-irradiated (i.e., absent (AB)) leaf packs deployed at “home” (H) and “away” (AW) sites tests the role of LAM  communities on home-field advantage patterns during leaf litter decomposition in rivers. We utilized a reciprocal transplant experiment between two temperate rivers, the Hoko and the Sekiu, on the Olympic Peninsula in Washington state. We subjected leaf packs to four conditions 1) P-H, 2) P-AW, 3) AB-H, and 4) AB-AW and asked the following questions: 
...
pacman::p_load('knitr', 'lme4', 'tidyverse', 'magrittr', 'effects', 'plyr', 'dplyr', 'plotrix', 'car',"lmerTest", "gridExtra", "cowplot", "tools", "tidymv", "ggthemes", "ggpubr", "scales", "emmeans", "fBasics", "MASS")
...
#add data files
#day 1
meta.day1.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_1_450_clean.csv")
#day 3
meta.day3.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_3_450_clean.csv")
#day 5
meta.day5.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_5_450_clean.csv")
#day 7
meta.day7.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_7_450_clean.csv")
#day 11
meta.day11.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_11_450_clean.csv")
#day 17
meta.day17.450.df<- read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/assay/day_17_450_clean.csv")
...
#decomp_day as factor
true_home.16.meta.df$decomp_day<- factor(true_home.16.meta.df$decomp_day) 
...
#double check log transformed normality
true_home.16.meta.df%>%
  group_by(treatment, tree_id, deploy, metabolite, site_num, tree_num, decomp_day, group)%>%
  pull(trans_od)%>%
  normalTest(method = "da")
...
#compare
AIC(meta_bigmod, meta_bigmod2, meta_bigmod3)

#can we remove any predictors
step(meta_bigmod3)

#even though the step function removes deploy, there is inherent variation——and the difference in df and AIC is small. We will keep random effect deploy
meta_bestmod<- lmer(log_od ~ treatment * group + (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num) , data = true_home.16.meta.df)
...
#create a null mod
meta_nullmod<- lmer(log_od ~ (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num) , data = true_home.16.meta.df)

#compare the best vs null
anova(meta_nullmod, meta_bestmod)

#mod summary
summary(meta_bestmod)
exp(coef(summary(meta_bestmod)))

#estimate marginal means
##we need to add decomp_day as a fixed effect to see pairwise
trt_means <- emmeans(meta_bestmod, ~ treatment:group)
summary(marg_means)
pairs(marg_means)
...
#filter out rows containing "AB AW"
filtered_decomp_pairs <- decomp_pairs_df %>%
  filter(!grepl("AB AW", contrast))

View(filtered_decomp_pairs)
```
...
#boxplot 
true_home.16.meta.df %>%
  ggplot(aes(x = group, y = trans_od, fill = treatment, color = treatment, alpha = 0.4)) +
  geom_point(aes(color = treatment), alpha = 0.4)+
  geom_boxplot(alpha = 0.4)+
  facet_grid(~decomp_day)+
...
#mean with SE 
#global mean across days
mean_trans_od <- mean(true_home.16.meta.df$trans_od, na.rm = TRUE)

#big interaction plot
...
### just p——h vs aw plot
meta.ponly_plot<- true_home.16.meta.df %>%
  filter(treatment == "P") %>%
...
ggsave("figures/meta.ponly_plot.png", meta.ponly_plot, width = 14, height = 7)
...
#normality test
hist(true_home.zscore.df$z_score)
...
data exploration
```{r}
#visualize present distribution
merged.true_home_z.df%>%
  filter(treatment == "P" & group == "H")%>%
  mutate(trans.od = log(od_corrected_b_n))%>%
  pull(trans.od)%>%
  hist()
...
merged.true_home_z.df<- merged.true_home_z.df%>%
  group_by(treatment, tree_id, deploy, metabolite, site_num, tree_num, decomp_day)%>%
  mutate(trans.od = log(od_corrected_b_n))

#we need to check the normality for each decomposition day since the comparison is between each treatment within day e.g., day-1 P vs AB; day-3 P vs AB

#day 1 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "1",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "1",]$trans.od, method = "da")

normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "1",]$z_score, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "1",]$z_score, method = "da")

#day 3 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "3",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "3",]$trans.od, method = "da")

#day 5 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "5",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "5",]$trans.od, method = "da")

#day 7 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "7",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "7",]$trans.od, method = "da")

#day 11 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "11",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "11",]$trans.od, method = "da")

#day 17 normality
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "P" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "17",]$trans.od, method = "da")
normalTest(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB" & merged.true_home_z.df$group == "H" & merged.true_home_z.df$decomp_day== "17",]$trans.od, method = "da")
...
```{r}
#day 1
day1.lm<- lm(trans.od ~ treatment, subset = decomp_day == "1" & group == "H", data = merged.true_home_z.df)
plot(day1.lm)
summary(day1.lm)

step(day1.lm)

day1.lmer<- lmer(trans.od ~ treatment + (1| tree_num) + (1| site_num) + (1 | deploy) + (1 | tree_id), subset = decomp_day == "1" & group == "H", data = merged.true_home_z.df)

day1.lmer<- lmer(trans.od ~ treatment +  (1 | tree_num), subset = decomp_day == "1" & group == "H", data = merged.true_home_z.df)

AIC(day1.lm, day1.lmer)
...
#true home using transformed using mean and confidence intervals
meta.mean.ci.plot<- merged.true_home_z.df %>%
  na.omit() %>%
  filter(group == "H") %>%
  ggplot(aes(x = as.factor(decomp_day), y = trans.od, fill = treatment)) +
  stat_summary(fun = mean, geom = "point", aes(color = treatment), position = position_dodge(width = 0.5), size = 3) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, aes(color = treatment), position = position_dodge(width = 0.5)) +
  xlab("decomposition day") +
  ggtitle("") +
  ylab(expression("log optical density (OD"[450~nm]*")")) +
  scale_color_manual(values = c("#CC79A7", "#009E73")) +
...
ggsave("figures/meta.mean.ci.plot.pdf", meta.mean.ci.plot, width = 14, height = 7)

#day 1
compare.day1 <- merged.true_home_z.df %>%
  filter(group == "H", decomp_day == "1")

day1.lm<- lm(trans.od ~ treatment, data = compare.day1)
plot(day1.lm)
...
day3.lm<- lm(trans.od ~ treatment, data = compare.day3)
plot(day3.lm)
...
day5.lm<- lm(trans.od ~ treatment, data = compare.day5)
plot(day5.lm)
summary(day5.lm)

day5.bigmod<- lm(trans.od ~ treatment, data = compare.day5)


#day 7
compare.day7 <- merged.true_home_z.df %>%
  filter(group == "H", decomp_day == "7")

day7.lm<- lm(trans.od ~ treatment, data = compare.day7)
plot(day7.lm)
summary(day7.lm)

#day 11
compare.day11 <- merged.true_home_z.df %>%
  filter(group == "H", decomp_day == "11")

day11.lm<- lm(trans.od ~ treatment, data = compare.day11)
plot(day11.lm)
summary(day11.lm)

#day 17
compare.day17 <- merged.true_home_z.df %>%
  filter(group == "H", decomp_day == "17")

day17.lm<- lm(trans.od ~ treatment, data = compare.day17)
plot(day17.lm)
summary(day17.lm)
...
merged.true_home_z.df%>%
  filter(group =="H")%>%
  ggplot(aes(x= decomp_day, y= trans.od, color = treatment))+
  geom_point(aes(y= trans.od))+
  geom_smooth(method = "loess", formula = y~x)

bigmod<- lm(trans.od ~ treatment + tree_num + site_num, subset = group == "H", data = merged.true_home_z.df)
plot(bigmod)
step(bigmod)
vif(bigmod)
summary(bigmod)

smallmod<- lm(trans.od ~ treatment, subset = group == "H", data = merged.true_home_z.df)

AIC(bigmod, smallmod)

install.packages("pwr")
library(pwr)
pwr.t.test(d = 0.6, sig.level = 0.5, power = 0.8)
pwr.t.test(d = 0.6, sig.level = 0.5, power = 0.8)



lmermod<- lmer(formula = trans.od ~ treatment + (1 | tree_num) + (1|site_num), subset = group == "H", data = merged.true_home_z.df)
plot(lmermod)
AIC(bigmod, lmermod)
lmeraov<- Anova(lmermod)

TukeyHSD(aov(formula = trans.od ~ treatment + group + tree_num + treatment:group, 
    data = merged.true_home_z.df))

library(QuantPsyc)

vif(bestmod, type = "predictor")
...
library(stringr)
library(purrr)
mass.df=read.csv("/Users/cjspiegel/Desktop/GitHub/RAD Alder/data/leaf_mass.csv")
...
hist(mass.df[mass.df$treatment == "P" & mass.df$hfa == "H",]$perc.mass_loss)
hist(mass.df[mass.df$treatment == "P" & mass.df$hfa == "A",]$perc.mass_loss)
hist(mass.df[mass.df$treatment == "AB" & mass.df$hfa == "H",]$perc.mass_loss)
hist(mass.df[mass.df$treatment == "P" & mass.df$hfa == "A",]$perc.mass_loss)
...
mass.deploy.merged.df <- mass.deploy.merged.df %>%
  na.omit(c("initial_mass", "final_correct")) %>%
  mutate(k = -1/19 * log(final_correct / initial_mass))
...
mass.deploy.merged.df%>%
  filter(treatment == "P")%>%
ggplot(aes(x = hfa, y = z_score, fill= treatment)) +
  geom_boxplot(alpha= 0.5) +
  geom_point(aes(shape = as.factor(site_num)), position = position_jitter(width = 0.2)) +
  ggtitle("Z score mass loss")+
  scale_fill_manual(values  = c("#CC79A7", "#009E73"))+
...
#plot
hfai.allsites.plot<- hfai.allsites_removed %>%
  filter(!is.na(values)) %>%
...
isotope.df=isotope.df%>%
  dplyr::group_by(treatment, tree_id, site_num, tree_num)%>%
    dplyr::mutate(total_C.g=total_C/10^6)%>%
    dplyr::mutate(total_N.g=total_N/10^6)%>%
    dplyr::mutate(c_n = total_C.g / total_N.g)%>%
   dplyr:: mutate(weight.g = weight/1000)%>%
   dplyr:: mutate(perc.C= (total_C.g/weight.g)*100)%>%
    dplyr::mutate(perc.N= (total_N.g/weight.g)*100)
View(isotope.df)
...
result_df <- merged_df %>%
  select(row.ID, compound_name, compound_class, star