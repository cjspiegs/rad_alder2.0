...
summary(meta_home_add_decompmod)
anova(meta_home_add_decompmod)

day3<- lmer(log_od ~ treatment + (1 | deploy) + (1 | site_num:tree_num), subset= group == "H" & decomp_day == "3", data = true_home.16.meta.df)
...
exp(0.50589 + 0.38305)
#just P
exp(0.50589 + 0.38305) - exp(0.38305)
#additive or not
1.466751+0.9657984

summary(meta_home_add_decompmod)

mean(true_home.16.meta.df[true_home.16.meta.df$treatment == "P",]$od_corrected_b_n)
mean(true_home.16.meta.df[true_home.16.meta.df$treatment == "AB",]$od_corrected_b_n)
((1.41-1.24)/1.41)*100

# Assuming log_fold_change has already been calculated
log_fold_change <- mean(true_home.16.meta.df[true_home.16.meta.df$treatment == "P",]$log_od)-
mean(true_home.16.meta.df[true_home.16.meta.df$treatment == "AB",]$log_od)

#effect of just aquatic microbes
exp(0.02588)
#1.026218

#effect of present and absent
exp(0.21858) + exp(0.02588)
#2.270526

#effect of just present microbes
(exp(0.21858) + exp(0.02588)) - exp(0.02588)
#1.244309
...
# Convert the fold change to a percentage
percentage_increase <- (fold_change - 1) * 100
percentage_increase
...
#across decomposition mean plot
meta.homeonly_plot<- true_home.16.meta.df%>%
  ...
  geom_hline(aes(yintercept = mean(log_od)), color = "red3", linetype = "dashed") +
  labs(color = "Treatment", shape = "Treatment", x = "Decomposition day", y = expression("Log(OD"[450 * nm]*")")) +
...
#across decomposition boxplot
meta.homeonly_trt.boxplot<- true_home.16.meta.df %>%
  filter(group == "H") %>%
  ggplot(aes(x = as.factor(decomp_day), y = od_corrected_b_n, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.6), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.6), size = 2, alpha = 0.6) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.6), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("resident absence", "resident presence")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("resident absence", "resident presence")) +
  labs(fill = "Treatment", shape = "Treatment", x = "Decomposition day", y = expression("Microbial metabolization (OD"[450 * nm]*")")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
ggsave("figures/meta.homeonly_trt.boxplot.png", meta.homeonly_trt.boxplot, width = 14, height = 7)
ggsave("figures/meta.homeonly_trt.boxplot.pdf", meta.homeonly_trt.boxplot, width = 14, height = 7)

#treatment
meta.homeonly_trt_plot<- true_home.16.meta.df%>%
  filter(group == "H") %>%
  ggplot(aes(x = as.factor(treatment), y = log_od, color = treatment, shape = treatment)) +
  stat_summary(fun = "mean", geom = "point", size = 6, position = position_dodge(width = 0.5)) +
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 0.4, position = position_dodge(width = 0.5), size = 1.2) +
  scale_color_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  labs(color = "Treatment", shape = "Treatment", x = "Treatment", y = expression("Log(OD"[450 * nm]*")")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16))))+
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#mean plot
meta.homeonly_trt_meanplot<- true_home.16.meta.df %>%
  filter(group == "H") %>%
  ggplot(aes(x = as.factor(treatment), y = od_corrected_b_n, color = treatment, shape = treatment)) +
  geom_point(position = position_jitter(width = 0.15), size = 1.5, alpha = 0.5) + # Smaller points with transparency
  stat_summary(fun = "mean", geom = "point", size = 4, position = position_dodge(width = 0.5)) + # Slightly smaller summary points
  stat_summary(fun.data = "mean_cl_normal", geom = "errorbar", width = 0.2, position = position_dodge(width = 0.5), size = 1.2) + # Reduced width of error bars
  scale_color_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  labs(color = "Treatment", shape = "Treatment", x = "Treatment", y = expression("Microbial metabolization (OD"[450 * nm]*")")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#boxplot trt only
meta.homeonly_trt_boxplot<- true_home.16.meta.df %>%
  filter(group == "H") %>%
  ggplot(aes(x = as.factor(treatment), y = od_corrected_b_n, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("Microbial metabolization (OD"[450 * nm]*")")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
ggsave("figures/meta.homeonly_trt_boxplot.png", meta.homeonly_trt_boxplot, width = 7, height = 5)
...
meta.p_only_plot<- true_home.16.meta.df %>%
  ...
  stat_summary(fun = mean, geom = "point", size = 6, position = position_dodge(width = 0.5)) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, position = position_dodge(width = 0.2), size = 1.2) +
...
normalTest(absent_rm, method = "da")
#removed model
...
#boxplot
hfai.boxplot<- hfai.allsites_removed %>%
  filter(!is.na(values)) %>%
ggplot(aes(x = treatment, y = values, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("resident absence", "resident presence")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("resident absence", "resident presence")) +
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("% Faster decomposition at home")) +
  ...
  theme(axis.text = element_text(size = 18),
        ...
        legend.title = element_text(size = 18))
hfai.boxplot
ggsave("figures/hfai.boxplot.pdf", hfai.boxplot, width = 8, height = 6)
ggsave("figures/hfai.boxplot.png", hfai.boxplot, width = 8, height = 6)
...
ggplot(aes(x = treatment, y = values, color = treatment, shape = treatment)) +
    stat_summary(fun = "mean", geom = "point", size = 6, position = position_dodge(width = 0.5), color = "red2") +
...
#bar plot
hfai.allsites_removed.barplot<- hfai.allsites_removed%>%
  filter(!is.na(values)) %>%
  ggplot(aes(x = reorder(names, values), y = values, fill = treatment, width = 0.85)) +
scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  labs(x = "", y = "Magnitude of decomposition (%)", fill = "Treatment") +
...
ggsave("figures/hfai.allsites_removed.barplot.png", hfai.allsites_removed.barplot, width = 8, height = 6)
...
group_P<- hfai.allsites_removed[hfai.allsites_removed$treatment == "P",]$values
group_AB<- hfai.allsites_removed[hfai.allsites_removed$treatment == "AB",]$values
...
### load negative mode
neg_data <- read_csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/day0_newprotocol_08292024/NEG_day0_newprotocol_trans_quant.csv") %>% arrange(Sample)
View(neg_data)
neg_metadata <- read_csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/day0_newprotocol_08292024/RAD_prelim_metadata_negative_rstudio.csv") %>% arrange(Sample)
...
```
days 3, 7, 19 chemistry
```{r}
#load quant data
all.chem.days.df<- read.csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/all_days/mzmine_output/all_days_negative_alder_2024_quant.csv")

#create HFA column
all.chem.days.df<- all.chem.days.df %>%
  mutate(hfa = ifelse(origin == "HOK" & deploy_site == "HOK", "home", 
            ifelse(origin == "HOK" & deploy_site == "SEK", "away", 
            ifelse(origin == "SEK" & deploy_site == "SEK", "home", 
            ifelse(origin == "SEK" & deploy_site == "HOK", "away", NA)))))%>%
  dplyr::select(1:10, hfa, everything())

#reshape so ms and intensity are in rows
all.chem.days.df<- all.chem.days.df %>%
  pivot_longer(
    cols = starts_with("MS_"),      # Select all MS columns
    names_to = "ms",                # Create a new column 'ms' with the MS_ labels
    values_to = "intensity"         # Create a new column 'intensity' for the peak intensities
  )


#load gnps library results
gnps_all.chem.days.df<- read.csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/all_days/mzmine_output/edited_library_results_2024.csv")

#create new df with annotations
annotated.df<- all.chem.days.df %>%
  left_join(gnps_all.chem.days.df, by = "ms")

#add log trans column
...
annotated.df%>%
  na.omit()%>%
  filter(log.intensity > 0 & hfa == "home")%>%
  ggplot(aes(x = as.numeric(day), y = log.intensity))+
  geom_smooth(aes(fill = treatment))+
  facet_wrap(~np_superclass)+
  theme_bw()
...
```{r}
### load negative mode
neg_data.df <- read_csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/day0_newprotocol_08292024/NEG_day0_newprotocol_trans_quant.csv") %>% arrange(Sample)
#add MS to column names
neg_data.df<- neg_data.df %>%
  rename_with(~ paste0("MS_", .), -1)

#metadata
neg_metadata <- read_csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/day0_newprotocol_08292024/RAD_prelim_metadata_negative_rstudio.csv") %>% arrange(Sample)
all(neg_data.df$Sample == neg_metadata$Sample)
neg_metadata$Sample <- gsub("_Neg", "", neg_metadata$Sample)

#add feature list
features.df<- read.csv("/Users/cjspiegel/Desktop/Projects/rad_alder/alder_metabolomics_2024/Negative/day0_newprotocol_08292024/feature_list.csv")
features.df<- features.df %>%
  mutate(ms = paste0("MS_", ms))

#make long
neg_data_long.df<- neg_data.df %>%
  pivot_longer(cols = starts_with("MS_"), 
               names_to = "ms", 
               values_to = "Intensity")

#merge
merged_df<- merge(neg_data_long.df,neg_metadata, by = "Sample")
merged_df<- merge(merged_df,features.df, by = "ms")

#filter compounds of interest
merged_filtered.df<- merged_df%>%
  filter(npclassifier_superclass == "Diarylheptanoids" | npclassifier_superclass == "Phenolic acids (C6-C1)" | npclassifier_superclass == "Flavonoids" | npclassifier_superclass == "Lignans" |
           npclassifier_superclass == "Aromatic polyketides" |
           npclassifier_superclass == "Phenylpropanoids (C6-C3)" |
           npclassifier_superclass == "Apocarotenoids" |
           npclassifier_superclass == "Saccharides")

#shift values to positive
merged_filtered.df<- merged_filtered.df%>%
  mutate(log_trans = log(Intensity + 1))

#how many features:
count_summary <- merged_filtered.df %>%
  dplyr::group_by(npclassifier_superclass, ATTRIBUTE_type) %>%
  dplyr::summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = ATTRIBUTE_type, values_from = count, values_fill = 0)
count_summary

#custom labels
custom_facet_labels <- c(
  "Diarylheptanoids" = "Diarylheptanoids\n n = 220",
  "Phenolic acids (C6-C1)" = "Phenolic Acids\n n = 320 ",
  "Flavonoids" = "Flavonoids\n n = 280",
  "Lignans" = "Lignans\n n = 60",
  "Phenylpropanoids (C6-C3)" = "Phenylpropanoids\n n = 100",
...
x_axis_labels<- c("absence", "presence")

### USE THIS ONE ####
half_boxviolin.plot<- merged_filtered.df%>%
  filter(npclassifier_superclass != "Aromatic polyketides" & log_trans > 0) %>%  # Filter out Aromatic polyketides and keep log_trans > 0
   ggplot(aes(x = ATTRIBUTE_type, y = log_trans, fill = npclassifier_superclass))+
  geom_boxplot(position = position_dodge(width = 0.6), alpha = 0.85, width = 0.45) +
        geom_flat_violin(alpha = 0.3, position = position_dodge(width = 0.5))+
  scale_fill_viridis_d(labels = custom_legend_labels) +
  scale_x_discrete(labels = x_axis_labels, position = "bottom")+
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.6), color = "white", alpha = 0.9) + 
  labs(fill = "Compound Superclass", shape = "Compound Superclass", x = "Treatment", y = expression("Log(Peak Intensity)")) +
  facet_grid(~npclassifier_superclass, labeller = labeller(npclassifier_superclass = custom_facet_labels))+
  theme_bw() +
...
ggsave("figures/half_boxviolin.plot.pdf", half_boxviolin.plot, width = 20, height = 8)

#Perform t-tests
t_test_results <- merged_filtered.df %>%
  dplyr::group_by(npclassifier_superclass) %>%
  dplyr::summarise(
    p_value = t.test(log_trans[ATTRIBUTE_type == "P"], 
                     log_trans[ATTRIBUTE_type == "AB"], 
...
###apply box cox model
model <- lm(shifted_values ~ ATTRIBUTE_type, data = merged_filtered.df)
...
#apply boxcox
merged_filtered.df <- merged_filtered.df %>%
  group_by(Sample, ms, ATTRIBUTE_type, ATTRIBUTE_Treenum, ATTRIBUTE_TreeID, superclass, class, subclass, npclassifier_superclass, npclassifier_class, npclassifier_pathway)%>%
  mutate(boxcox = ifelse(lambda_opt == 0,
                                     log(shifted_values), 
                                     (shifted_values^lambda_opt - 1) / lambda_opt))%>%
  ungroup()

#summarize df
fold_change_summary <- merged_filtered.df %>%
  na.omit() %>%
  dplyr::group_by(npclassifier_superclass) %>%
  dplyr::summarise(
    # Calculate the fold change
    fold_change = log2(mean(boxcox[ATTRIBUTE_type == "P"], na.rm = TRUE) / 
                       mean(boxcox[ATTRIBUTE_type == "AB"], na.rm = TRUE)),
    
    # Perform t-test and extract p-value
    p_value = t.test(boxcox[ATTRIBUTE_type == "P"], 
                     boxcox[ATTRIBUTE_type == "AB"])$p.value,
    
    # Assign significance based on p-value
...
#plot
fold_change.plot<- ggplot(fold_change_summary, aes(x = fold_change, y = npclassifier_superclass)) +
  geom_point(aes(color = sig), size = 4) +  
  geom_vline(xintercept = 0, linetype = "dashed", color = "red3", size = 1) +  
  scale_color_manual(values = c("significant" = "red", "not significant" = "black")) + 

  coord_cartesian(xlim = c(-0.1, 0.15)) +  
  labs(color = "Significance", 
       x = "Log2 Fold Change", y = "Compound") +  
  theme_linedraw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
        legend.text = element_text(size = 18),
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.grid.major.y = element_line(color = "lightgray", size = 0.5), legend.position = "none")

ggsave("figures/fold_change.plot.png", fold_change.plot, width = 10, height = 10)
ggsave("figures/fold_change.plot.pdf", fold_change.plot, width = 10, height = 10)
...
box_cox_transform <- function(x) {
  bc <- boxcox(x ~ 1, lambda = seq(-2, 2, 0.1), plotit = FALSE)
...
merged_filtered.df <- merged_filtered.df %>%
  ...
  mutate(boxcox_intensity = box_cox_transform(shifted_intensity)) %>%
  ungroup()

merged_filtered.df <- merged_filtered.df %>%
  group_by(npclassifier_superclass) %>%
  mutate(
    normalized_intensity = (boxcox_intensity - mean(boxcox_intensity)) / sd(boxcox_intensity)
  ) %>%
  ungroup()

merged_filtered.df%>%
  pull(normalized_intensity)%>%
  normalTest(method = "da")

# Step 3: Log transformation (base 10)
merged_filtered.df <- merged_filtered.df %>%
  mutate(trans_values = log(shifted_intensity))  # Adding 1 to handle zero values before log transformation
...
hist(merged_filtered.df$trans_values)
...
# Group by npclassifier_superclass and summarize the counts for P and AB
count_summary <- merged_filtered.df %>%
  dplyr::group_by(npclassifier_superclass, ATTRIBUTE_type) %>%
  dplyr::summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = ATTRIBUTE_type, values_from = count, values_fill = 0)
...
# Calculate the mean difference for each compound superclass
difference_df <- merged_filtered.df %>%
  dplyr::group_by(npclassifier_superclass, ATTRIBUTE_type) %>%
  dplyr::summarise(mean_trans_value = mean(trans_values, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = ATTRIBUTE_type, values_from = mean_trans_value) %>%
  mutate(difference = P-AB)

# Perform t-tests
t_test_results <- merged_filtered.df %>%
  dplyr::group_by(npclassifier_superclass) %>%
  dplyr::summarise(
    p_value = t.test(trans_values[ATTRIBUTE_type == "P"], 
                     trans_values[ATTRIBUTE_type == "AB"], 
                     var.equal = FALSE)$p.value,
    .groups = "drop"
  ) %>%
  mutate(significant = p_value < 0.05)
...
# Combine results
final_df <- difference_df %>%
  left_join(t_test_results, by = "npclassifier_superclass")

# Create the plot
ggplot(final_df, aes(x = difference, y = reorder(npclassifier_superclass, difference))) +
  geom_point(aes(color = significant), size = 4) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") +
  scale_color_manual(values = c("black", "red"), 
                     labels = c("")) +
  labs(x = "Difference in Transformed Values (P - AB)", 
       y = "Compound Superclass",
       title = "Mean Difference in Metabolite Levels Between P and AB",
       color = "") +
  theme_minimal() +
...
#fold change
merged_filtered.df <- merged_filtered.df %>%
  group_by(ms, Sample, npclassifier_superclass) %>%
  mutate(fold_change = log2(
      shifted_intensity[ATTRIBUTE_type == "P"] / 
      shifted_intensity[ATTRIBUTE_type == "AB"]
  )) %>%
  summarise(
      p_value = t.test(
          shifted_intensity[ATTRIBUTE_type == "P"],
  ...
  mutate(sig = ifelse(p_value > 0.05, "not significant", "significant"))



#log trans
merged_filtered.df$log_change <- log((merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "P",]$shifted_intensity)/(merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "AB",]$shifted_intensity))


#fold change plot
#treatment
fold_change.trt.plot<- merged_filtered.df %>%
  na.omit()%>%
  ggplot(aes(x = fold_change, y = npclassifier_superclass, color = ATTRIBUTE_type)) +
  stat_summary(fun = mean, geom = "point", size = 4) +
  scale_color_manual(values = c("AB" = "grey60", "P" = "black"), labels = c("resident absence", "resident presence")) + 
  geom_vline(xintercept = 0, linetype = "dashed", color = "red3", size = 1) +  # Add vertical dashed line at x = 0
  labs(color = "Treatment", 
       x = "Log2 Fold Change", y = "Compound") +  # Update axis labels
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
        legend.text = element_text(size = 18),
...
final_df %>%
  na.omit() %>%
  ggplot(aes(x = difference, y = npclassifier_superclass)) +  # fold_change as x-axis, compound on y-axis
  geom_point(size = 4, shape = 21, fill = "grey60") +  # Single point per compound
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
        legend.text = element_text(size = 18),
...
ggsave("figures/fold_change.plot.png", fold_change.plot, width = 9, height = 8)
ggsave("figures/fold_change.plot.pdf", fold_change.plot, width = 9, height = 8)

#site
merged_filtered.df %>%
  na.omit()%>%
  ggplot(aes(x = fold_change, y = npclassifier_superclass, color = ATTRIBUTE_TreeID, shape = ATTRIBUTE_Treenum)) +
  stat_summary(fun = mean, geom = "point", size = 4) +
  scale_color_manual(values = c("HOK" = "grey60", "SEK" = "black"), labels = c("Hoko", "Sekiu")) + 
...
merged_filtered.df %>%
  na.omit()%>%
  ggplot(aes(y = log(Intensity), x = npclassifier_superclass, color = ATTRIBUTE_type)) +
...
hist(log(merged_filtered.df$Intensity))
...
merged_filtered.df%>%
   ggplot(aes(x = ATTRIBUTE_type, y = log_trans, fill = npclassifier_superclass, shape = npclassifier_superclass))+
  geom_flat_violin()+
  geom_point(aes(color = npclassifier_superclass), 
...
```{r}
###apply box cox model
model <- lm(Inte ~ ATTRIBUTE_type, data = df_filtered)
...
merged_df <- merged_df %>%
  ...
  mutate(boxcox = ifelse(lambda_opt == 0,
                                     log(Intensity), 
                                     (Intensity^lambda_opt - 1) / lambda_opt))%>%
  ungroup()
...
#shift values to positive
df_final<- df_final%>%
  mutate(shifted_values = values + 1)

#add a log transformation
df_final<- df_final%>%
    group_by(Sample, ms, ATTRIBUTE_type,ATTRIBUTE_Sitenum, ATTRIBUTE_TreeID, ATTRIBUTE_Treenum ) %>%
  mutate(log_trans = log(shifted_values))

#Normalize by median within each ms group
df_final <- df_final %>%
  group_by( ms ) %>%
  mutate(trans_values = shifted_values / median(shifted_values, na.rm = TRUE))
...
###apply box cox model
model <- lm(shifted_values ~ ATTRIBUTE_type, data = df_filtered)
...
df_filtered <- df_filtered %>%
  group_by(Sample, ms, ATTRIBUTE_type, ATTRIBUTE_Treenum, ATTRIBUTE_TreeID)%>%
  mutate(boxcox = ifelse(lambda_opt == 0,
                                     log(shifted_values), 
                                     (shifted_values^lambda_opt - 1) / lambda_opt))%>%
  ungroup()
...
#check dist.
hist(df_filtered$boxcox)
min(df_filtered$boxcox)
max(df_filtered$boxcox)

#within treatment
hist(df_filtered[df_filtered$ATTRIBUTE_type == "AB",]$boxcox)

hist(log(df_filtered[df_filtered$ATTRIBUTE_type == "AB",]$boxcox))
...
filtered_df <- df_final[, c(columns_to_keep)]
...
# Merge with the mapping table to add the names column
df_filtered <- merge(df_filtered, ms_names_mapping, by.x = "ms", by.y = "MS")
...
# Columns to keep
columns_to_keep <- c("Sample", "ATTRIBUTE_type", "ATTRIBUTE_TreeID", "ATTRIBUTE_Sitenum", "ATTRIBUTE_Treenum",
                     "MS_363", "MS_371", "MS_372", "MS_369", "MS_475", "MS_646", "MS_752", "MS_754", "MS_823",
                     "MS_834", "MS_895", "MS_929", "MS_1141", "MS_1203", "MS_1205", "MS_1240", "MS_1297",
                     "MS_1300", "MS_1422", "MS_1431", "MS_1442", "MS_1456")

# Check and keep only the existing columns
existing_columns_to_keep <- columns_to_keep[columns_to_keep %in% colnames(df_filtered)]

# Subset the dataframe
filtered_df <- df_filtered[, c(existing_columns_to_keep, "names")]
...
#plot
#mean plot
day0_chem_mean.plot<- df_filtered %>%
...
#boxplot
day0_chem_boxplot<- df_filtered %>%
  ggplot(aes(x = as.factor(ATTRIBUTE_type), y = boxcox, fill = ATTRIBUTE_type, shape = ATTRIBUTE_type, na.omit = T)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
      facet_wrap(.~names)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("Boxcox transformed values")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
ggsave("figures/day0_chem_boxplot.png", day0_chem_boxplot, width = 14, height = 7)


# Subset the data for "Linear diarylheptanoids"
hist(log(df_filtered[df_filtered$names == "Linear diarylheptanoids",]$values)+1)
compound <- "Linear diarylheptanoids"
df_sub <- df_filtered[df_filtered$names == compound, ]
t.test(boxcox ~ ATTRIBUTE_type, data = df_sub)


# Subset the data for "Gallotannins"
hist(sqrt(df_filtered[df_filtered$names == "Gallotannins",]$trans_values))
...
hist(log(df_filtered[df_filtered$names.x == "Shikimic acids and derivatives",]$values))
...
```{r}
#load data
isotope.df<- read.csv("/Users/cjspiegel/Desktop/Projects/rad_alder/isotopes/alder_isotopes_0_7_19_2024.csv")

#normalize and create c:n column
isotope.df<- isotope.df%>%
  group_by(treatment, origin_tree, origin_site, tree_num, deploy_site, deploy_num, day)%>%
   mutate(
     totalC.mg = totalC_ug / 10^3,
          totalC.weight = totalC.mg/sample_weight_mg,
          totalN.mg = totalN_ug / 10^3,
          totalN.weight = totalN.mg/sample_weight_mg,
         c_n = totalC.weight / totalN.weight,
        log.c_n = log(c_n),
         perc.c = (totalC.weight) * 100,
         perc.n = (totalN.weight) * 100)

isotope.df$scaled.c_n<- scale(isotope.df$c_n)
isotope.df$scaled.d15N<- scale(isotope.df$d15N)
isotope.df$scaled.d13C<- scale(isotope.df$d13C)

#assign home and away
isotope.df$group <- 
  ifelse(
  isotope.df$origin_tree == "HOK" & isotope.df$deploy_site == "HOK", "H",
   ifelse(
  isotope.df$origin_tree == "SEK" & isotope.df$deploy_site == "SEK", "H",
   ifelse(
   isotope.df$origin_tree == "HOK" & isotope.df$deploy_site == "SEK", "AW",
   ifelse(
   isotope.df$origin_tree == "SEK" & isotope.df$deploy_site == "HOK", "AW", NA  # Default value if none of the conditions are met
                )
              )
            )
          )
...
#data distribution
hist(log(isotope.df$c_n))
hist(isotope.df$scaled.c_n,breaks = 4)
  hist(isotope.df$scaled.c_n)
hist(scale(isotope.df$delta13C))
hist(scale(isotope.df$delta15N))
...
####C:N stats and plot
#mean plot
c_n_mean.plot<- isotope.df%>%
  ggplot(aes(treatment, log(c_n), color = treatment, shape = treatment))+
  stat_summary(fun = mean, geom = "point", size = 5, position = position_dodge(width = 0.5)) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, position = position_dodge(width = 0.5), size = 1) +
  scale_color_manual(values = c("AB" = "black", "P" = "grey60"), labels = c("absent", "present")) +
    scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
...
#boxplot
c_n_boxplot<- isotope.df %>%
  ggplot(aes(x = as.factor(treatment), y = log.c_n, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("C:N")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
ggsave("figures/c_n_boxplot.png", c_n_boxplot, width = 10, height = 8)

#delta 13 C boxplot
d13C.boxplot<- isotope.df %>%
  ggplot(aes(x = as.factor(treatment), y = d13C, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("delta 13C")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
isotope.df %>%
  filter(group == "H")%>%
  ggplot(aes(x = as.factor(treatment), y = scaled.c_n, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("scaled C:N")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#delta 15 N boxplot
d15N.boxplot<- isotope.df %>%
  ggplot(aes(x = as.factor(treatment), y = d15N, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("delta 15N")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#total C boxplot
perc.c.boxplot<- isotope.df %>%
    filter(group == "H")%>%
  ggplot(aes(x = as.factor(treatment), y = perc.c, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("% C")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#total N boxplot
perc.n.boxplot<- isotope.df %>%
  ggplot(aes(x = as.factor(treatment), y = perc.n, fill = treatment, shape = treatment)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("% N")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
isotope.df %>%
  filter(group == "H")%>%
  ggplot(aes(x = scaled.c_n, y = scaled.d15N, fill = treatment, shape = treatment)) +
  geom_smooth() +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
   # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
...
d15N_c_n_boxplot
isotope.df %>%
  ggplot(aes(x = scaled.d13C , y = scaled.d15N, fill = treatment, shape = treatment)) +
  ...
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "d13C", y = expression("d15N")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
#plot grid
all.days_isotope.plot<- plot_grid(c_n_boxplot, d13C.boxplot, d15N.boxplot, perc.n.boxplot, perc.c.boxplot, nrow= 5, rel_widths = c(3,3,3,3,3))

ggsave("figures/all.days_isotope.plot.png", all.days_isotope.plot, width = 12, height = 16)
...
per_n.boxplot<- isotope.df %>%
  ...
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
  facet_grid(~day)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("% Nitrogen")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
ggsave("figures/perc_n_boxplot.png", perc_n_boxplot, width = 10, height = 7)
...
perc.C.plot=isotope.df %>%
  ...
  xlab("treatment")+
  ylab("% C")+
  geom_point(aes(shape = as.factor(deploy_site)), position = position_jitter(width = 0.15))+
scale_color_manual(values = c("#CC79A7", "#009E73")) +
    scale_fill_manual(values = c("#CC79A7", "#009E73")) +
