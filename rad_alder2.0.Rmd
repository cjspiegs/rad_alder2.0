...
true_home.meta.df$metabolite<- true_home.meta.df(meta.450.df$metabolite)
...
#check out decomp day
meta_bigmod4<- lmer(log_od ~ treatment * group +  decomp_day  + (1 | deploy) + (1 | site_num:tree_num) , data = true_home.16.meta.df)

meta_bigmod5<- lmer(log_od ~ treatment * group * decomp_day + (1 | deploy) + (1 | site_num:tree_num) , data = true_home.16.meta.df)

#compare again
AIC(meta_bigmod3, meta_bigmod4, meta_bigmod5)
...
meta_bestmod<- glmer(od_corrected_b_n ~ treatment * group + (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num), subset = od_corrected_b_n>= 0, family = Gamma(link = "log"), data = true_home.16.meta.df)
#create a big null mod
meta_nullmod<- glmer(od_corrected_b_n ~  (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num), subset = od_corrected_b_n>= 0, family = Gamma(link = "log"), data = true_home.16.meta.df)

# Exponentiate the confidence intervals to get them back to the original scale
...
Mod for at home only
```{r}
##best home model
meta_homemod<- glmer(od_corrected_b_n ~ treatment + (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num), subset= group == "H", family = Gamma(link= "log"), data = true_home.16.meta.df)

#check the best home mod
plot(resid(meta_homemod))
hist(resid(meta_homemod))
normalTest(resid(meta_homemod), method = "da")

#create a home null mod
meta_homenullmod<- glmer(od_corrected_b_n ~ (1 | deploy) + (1 | decomp_day) + (1 | site_num:tree_num), subset= group == "H", family = Gamma(link= "log"), data = true_home.16.meta.df)

#compare the best vs null
anova(meta_homenullmod, meta_homemod)

#mod summary
summary(meta_homemod)
Anova(meta_homemod)
...
homemod_fixed <- fixef(meta_homemod)
se <- sqrt(diag(vcov(meta_homemod)))
#calculation
lower_ci <- exp(homemod_fixed - 1.96 * se)
lower_ci
upper_ci <- exp(homemod_fixed + 1.96 * se)
upper_ci

#estimate marginal means
#maybe we can see pairwise among decomp day
meta_home_decompmod<- glmer(od_corrected_b_n ~ treatment+ (1| decomp_day) + (1 | deploy) + (1 | site_num:tree_num), subset= group == "H", family = Gamma(link= "log"), data = true_home.16.meta.df)

meta_home_decompmod2<- glmer(od_corrected_b_n ~ treatment + decomp_day + (1 | deploy) + (1 | site_num:tree_num), subset= group == "H", family = Gamma(link= "log"), data = true_home.16.meta.df)

meta_home_decompmod3<- glmer(od_corrected_b_n ~ treatment * decomp_day + (1 | deploy) + (1 | site_num:tree_num), subset= group == "H", family = Gamma(link= "log"), data = true_home.16.meta.df)

AIC(meta_home_decompmod,meta_home_decompmod2,meta_home_decompmod3)
#check out mod
plot(resid(meta_home_decompmod3))
hist(resid(meta_home_decompmod3))
#summary
summary(meta_home_decompmod3)
#means
home_decomp_means <- emmeans(meta_home_decompmod3, ~ treatment : decomp_day, type = "response")
#get the pairwise comparisons for each decomp_day
home_decomp_pairs <- as.data.frame(pairs(home_decomp_means))
#add sig column
home_decomp_pairs$significant <- home_decomp_pairs$p.value < 0.05

##plot
ggplot(home_decomp_pairs, aes(x = contrast, y = ratio, ymin = ratio - SE, ymax = ratio + SE, color = significant)) +
  geom_pointrange() +
...
#convert decomp_pairs to df
home_pairs_df <- as.data.frame(home_decomp_pairs)
home_sig_pairs <- subset(home_pairs_df, p.value < 0.05)
home_sig_pairs
