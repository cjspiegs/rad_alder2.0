...
#log2 fold change
 merged_filtered.df$fold_change <- log2((merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "P",]$Intensity)/(merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "AB",]$Intensity))
 
#mean fold change
 merged_filtered.df%>%
   group_by(Sample)%>%
   mutate(mean_fold = mean(fold_change))
...
#fold change plot
merged_filtered.df %>%
  ggplot(aes(x = fold_change, y = npclassifier_superclass, color = npclassifier_superclass)) +
  geom_point(size = 3, alpha = 0.7) +  # Add dots representing metabolites
  scale_color_viridis_d() +  # Use Viridis color scale for better visibility
  scale_shape_manual(values = c("Diarylheptanoids" = 1, 
                                "Phenolic acids (C6-C1)" = 2, 
                                "Flavonoids" = 3, 
                                "Lignans" = 5, 
                                "Jasmonic acids" = 6, 
                                "Aromatic polyketides" = 7, 
                                "Phenylpropanoids (C6-C3)" = 8, 
                                "Apocarotenoids" = 9, 
                                "Saccharides" = 10)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red", size = 1) +  # Add vertical dashed line at x = 0
  labs(color = "Compound Superclass", shape = "Compound Superclass", 
       x = "Log Fold Change", y = "Metabolite") +  # Update axis labels
  theme_bw() +
  facet_wrap(~ATTRIBUTE_type)+
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
        legend.text = element_text(size = 18),
        legend.title = element_text(size = 18),
