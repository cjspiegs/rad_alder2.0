...
neg_long.df <- neg_long.df %>%
  group_by(ms) %>%
  mutate(values = values / median(values, na.rm = TRUE))

# Step 3: Log transformation (base 10)
neg_long.df <- neg_long.df %>%
  mutate(values = log10(values + 1))  # Adding 1 to handle zero values before log transformation
...
day0_chem.plot<- df_filtered %>%
  ggplot(aes(x = ATTRIBUTE_type, y = values, color = ATTRIBUTE_type, shape = ATTRIBUTE_type)) +
