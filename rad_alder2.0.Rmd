...
merged_filtered.df<- merged_df%>%
  filter(npclassifier_superclass == "Diarylheptanoids" | npclassifier_superclass == "Phenolic acids (C6-C1)" | npclassifier_superclass == "Flavonoids" | npclassifier_superclass == "Lignans" |
           npclassifier_superclass == "Jasmonic acids" |
...
half_boxviolin.plot<- merged_filtered.df%>%
  ...
  geom_boxplot(position = position_dodge(width = 0.6), alpha = 0.7, width = 0.45) +
        geom_flat_violin(alpha = 0.4, position = position_dodge(width = 0.5))+
  scale_fill_viridis_d() +
  scale_shape_manual(values = c("Diarylheptanoids" = 1, "Phenolic acids (C6-C1)" = 2, "Flavonoids" = 3, "Lignans" = 5 , "Jasmonic acids" = 6, "Aromatic polyketides" = 7 , "Phenylpropanoids (C6-C3)" = 8, "Apocarotenoids" = 9, "Saccharides" = 10))+
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.6), color = "red2", alpha = 0.9) + # Add mean points
  labs(fill = "Compound Superclass", shape = "Compound Superclass", x = "Treatment", y = expression("Log(Peak Intensity)")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  facet_grid(~npclassifier_superclass, space = "free_x")+
  theme_bw() +
...
ggsave("figures/half_boxviolin.plot.pdf", half_boxviolin.plot, width = 20, height = 6)
