...
merged_filtered.df<- merged_filtered.df%>%
  mutate(shifted_intensity = Intensity + 1)

#log2 fold change
 merged_filtered.df$fold_change <- log2((merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "P",]$shifted_intensity)/(merged_filtered.df[merged_filtered.df$ATTRIBUTE_type == "AB",]$shifted_intensity))
...
#fold change plot
fold_change.plot<- merged_filtered.df %>%
  na.omit()%>%
...
#plot
merged_filtered.df%>%
   ggplot(aes(x = ATTRIBUTE_type, y = log(Intensity), fill = npclassifier_superclass, shape = npclassifier_superclass))+
  geom_boxplot(position = position_dodge(width = 0.6), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.6), size = 2, alpha = 0.6) +
  ...
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  facet_grid(~npclassifier_superclass)+
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
        legend.text = element_text(size = 18),
