...
isotope.df %>%
  group_by(treatment, origin_tree, origin_site, tree_num, deploy_site, deploy_num, day)%>%
  mutate(
    # Extract day 0 values directly for each sample
    d13C_day0 = first(d13C[day ==0]),
    d15N_day0 = first(d15N[day == 0]),
    N_day0 = first(totalN.weight[day == 0]),
    C_day0 = first(totalC.weight[day == 0]),
    
    # Calculate percent change for each day based on day 0
    perc_change_d15N = ifelse(day == 0, 0, ((d15N - d15N_day0) / d15N_day0) * 100),
    perc_change_N = ifelse(day == 0, 0, ((totalN.weight - N_day0) / N_day0) * 100),
    perc_change_C = ifelse(day == 0, 0, ((totalC.weight - C_day0) / C_day0) * 100)
  ) %>%
  ungroup()
...
isotope.df$scaled_d13C<- scale(isotope.df$delta13C)
isotope.df$scaled_d15N<- scale(isotope.df$delta15N)
