...
#check min of data
if (any(neg_long.df$values <= 0)) {
  print("Data contains non-positive values.")
}

###apply box cox model
model <- lm(values ~ ATTRIBUTE_type, data = df_final)
...
hist(df_filtered[df_filtered$names == "Shikimic acids and derivatives",]$log)
...
df_filtered %>%
  ggplot(aes(x = as.factor(ATTRIBUTE_type), y = log, fill = ATTRIBUTE_type, shape = ATTRIBUTE_type, na.omit = T)) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7, width = 0.45) +
  geom_point(position = position_jitter(width = 0.1), size = 2, alpha = 0.4) + # Add jitter and reduce point size for raw data
  stat_summary(fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.75), color = "red2", alpha = 0.9) + # Add mean points
  scale_fill_manual(values = c("P" = "black", "AB" = "grey60"), labels = c("absent", "present")) +
  scale_shape_manual(values = c("AB" = 17, "P" = 16), labels = c("absent", "present")) +
      facet_wrap(.~names)+
  labs(fill = "Treatment", shape = "Treatment", x = "Treatment", y = expression("Log(relative abundance)")) +
  guides(color = guide_legend(override.aes = list(shape = c(17, 16)))) +
  theme_bw() +
  theme(axis.text = element_text(size = 18),
        axis.title = element_text(size = 18),
...
df_sub_clean %>%
  ...
  mutate(
    count = n(),
    unique_values = n_distinct(log),
    mean = mean(log, na.rm = TRUE),
    sd = sd(log, na.rm = TRUE),
    .groups = 'drop'
  )

t.test(log ~ ATTRIBUTE_type, data = df_sub)
...
df_sub <- df_filtered[df_filtered$names.x == compound, ]
# Perform t-test
t.test(values ~ ATTRIBUTE_type, data = df_sub)
